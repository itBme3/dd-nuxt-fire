<template>
  <div :class="{
      'site-nav left-0 top-0 fixed flex justify-space-between z-999999': true,
      'right-0 flex-col': isMobile,
      'right-0 w-screen': !isMobile
    }"
    >
    <div v-if="isMobile && !mobileNavCollapsed" @click="mobileNavCollapsed = true"
      class="cursor-pointer fixed inset-0 z-99999 bg-opacity-30 dark:bg-opacity-80 bg-gray-900"></div>
    <Btn v-if="isMobile"
      class="icon-button mt-1 mr-1 px-3 py-2 ml-auto dark:bg-gray-800 bg-white relative z-999999"
